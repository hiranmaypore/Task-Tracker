...
if (-not $login) {
    Write-Host "`n❌ LOGIN FAILED - Cannot continue tests" -ForegroundColor Red
    exit 1
}
...
$rule2 = Test-Endpoint -Name "Create Automation Rule (Urgent Keyword)" -Method POST -Path "/automation/rules" `
    -Headers $headers -Body @{
        trigger = "TASK_CREATED"
        conditions = @(
...
Test-Endpoint -Name "Get Automation Executions" -Method GET -Path "/analytics/automation/executions" `
    -Headers $headers -ExpectSuccess $false
...
# Note: Actual sync requires OAuth authorization, which needs manual browser interaction
# Test-Endpoint -Name "Sync Tasks to Calendar (Requires Auth)" -Method POST -Path "/calendar/sync" `
#     -Headers $headers -ExpectSuccess $false

# ============================================
# PHASE 9: REAL-TIME EVENTS (WebSocket)
# ============================================
Write-Host "`n=== PHASE 9: WEBSOCKET EVENTS ===" -ForegroundColor Magenta
Write-Host "  ℹ️  WebSocket testing requires separate client" -ForegroundColor Yellow
Write-Host "  Server is listening on ws://localhost:3000" -ForegroundColor Gray

# ============================================
# PHASE 10: CLEANUP & DELETION
# ============================================
Write-Host "`n=== PHASE 10: CLEANUP TESTS ===" -ForegroundColor Magenta
...
Write-Host "  ℹ️  WebSocket Events (Manual Test)" -ForegroundColor Yellow
Write-Host "  ℹ️  Analytics (Admin Required)" -ForegroundColor Yellow
